...
#define OFFIS_CONSOLE_APPLICATION "mppsrecv"
...
#define APPLICATIONTITLE "MPPSSCP"
...
int main(int argc, char *argv[])
{
    ...
    mppsSCP.setPort(OFstatic_cast(Uint16, opt_port));
    mppsSCP.setAETitle(opt_aeTitle);
    mppsSCP.setMaxReceivePDULength(OFstatic_cast(Uint32, opt_maxPDULength));
    mppsSCP.setACSETimeout(OFstatic_cast(Uint32, opt_acseTimeout));
    mppsSCP.setDIMSETimeout(OFstatic_cast(Uint32, opt_dimseTimeout));
    mppsSCP.setDIMSEBlockingMode(opt_blockingMode);
    mppsSCP.setVerbosePCMode(opt_showPresentationContexts);
    mppsSCP.setRespondWithCalledAETitle(opt_useCalledAETitle);
    mppsSCP.setHostLookupEnabled(opt_HostnameLookup);

    OFLOG_INFO(dcmrecvLogger, "starting service class provider and listening ...");

    /* start SCP and listen on the specified port */
    status = mppsSCP.listen();
    if (status.bad())
    {
